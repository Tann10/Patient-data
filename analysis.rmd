...
gat_null <- df %>% filter(is.na(GA_at_Observation))
...
df <- df %>% group_by(Dim_Patient) %>% 
  ...
  mutate(days_since = c(0,diff(date)), 
         number = (279-as.numeric(EDD) + as.numeric(date))/7, 
...
view(df %>% filter(Trimester != trimester))
```
...
daily <- df %>% group_by(Dim_Patient, date) %>% arrange(Dim_Patient, Observed_At) %>%
...
weekly <- daily %>% group_by(Dim_Patient, Trimester) %>% arrange(Dim_Patient, Observed_At) %>%
...
view(weekly %>% filter((days_since <= 70) & (abs(diff) >= 15)))
...
count_rows2 <- merge(count_rows, count_2, by = "Dim_Patient") 

count_rows2 <- count_rows2 %>% mutate(per_removed = ((initial_count - after_count)/initial_count) * 100)

low <- count_rows2 %>% filter(per_removed  > 70)
...
view(df %>% filter(Dim_Patient == "ref:Patient/BabyscriptsLegacy#8436"))
