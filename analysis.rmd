...
df <-subset(df,select = - c(Weight_Difference, Days_Since_Last_Entry, Observed_At))
...
low_count <-subset(low_count,select = - c(Observed_At))
...
df <- subset(df, select = -c(Trimester))
```
...
```{r}
p2 <- df %>% filter(Dim_Patient == "ref:Patient/BabyscriptsLegacy#3724")


  
daily <- df %>% group_by(Dim_Patient, date) %>% arrange(Dim_Patient, date) %>%
