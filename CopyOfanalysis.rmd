...
```{r}
intermediate_rollup <- df %>% group_by(Dim_Patient, Observed_At) %>% summarise(mean = mean(Observation_Value))
...
```{r}
p2 <- df %>% filter(Dim_Patient == "ref:Patient/BabyscriptsLegacy#2043")
...
low <- count_rows %>% filter(per_removed  > 70)

write.csv(low, "low_check2.csv")
