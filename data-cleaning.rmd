...
day_level_demo$bmi <- day_level_demo$weight_kg/(day_level_demo$`Height_(m)`^2)
...
```{r}
bmi <- day_level_demo %>% filter(!is.na(bmi_category))
...
q5_whole <- day_level_demo %>% group_by(Dim_Patient, trimester)  %>% 
  arrange(Dim_Patient,trimester,date) %>% filter(row_number()==1 | row_number()==n()) %>% 
  mutate(n = n()) %>%
  filter(n>1) %>%
  summarise(weight_diff = diff(wt_avg))

q5_prac <- day_level_demo %>% group_by(Customer_Group, Dim_Patient, trimester)  %>% 
 arrange(Customer_Group, Dim_Patient,trimester,date) %>% filter(row_number()==1 | row_number()==n()) %>% 
  mutate(n = n()) %>%
  filter(n>1) %>%
